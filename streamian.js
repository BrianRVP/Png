                                                                        /*| BFlix®TV for Movian/M7 Media Center | 2024 F0R3V3R50F7 |*/
        ...
        function processResults() {
            ...
            if (preferredResults.length > 0) {
                ...
                if (maxPreferredSeeders < 30) {
                    ...
                    if (selectedResult) {
                        popup.notify("BFlix®TV | Couldn't find a source in preferred quality, playing best source found.", 10);
                    }
                }
            } else {
                combinedResults.forEach(function(item) {
            ...
            if (selectedResult) {
                ...
                if (source === 'Archive.org') {
                    popup.notify("BFlix®TV | Streaming from " + source + " direct" + " at " + videoQuality, 10);
                } else {
                    popup.notify("BFlix®TV | Streaming from " + source + " with " + seederCount + " seeders" + " at " + videoQuality, 10);
                }
                checkCancellation();
...
new page.Route(plugin.id + ":start", function(page) {
    ...
    popup.notify('BFlix®TV | Raise Your BitTorrent Cache and #KeepTorrentsAlive | Is your favourite Movie or Show not here? Add it to TMDB yourself and watch it here!', 10);
...
new page.Route(plugin.id + ":play:(.*):(.*)", function(page, title, imdbid) {
    ...
    page.model.contents = 'list';
    popup.notify('BFlix®TV | Encountering issues? Please report to Reddit r/movian', 10);
